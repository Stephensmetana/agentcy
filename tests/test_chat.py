...
class TestUIServer:
    """Basic integration tests for the FastAPI UI server."""
    ...
    def test_get_messages_empty(self):
        ...
        try:
            ...
            app = create_ui_app(path, ROLES_PATH)
    ...
    def test_post_and_get_message(self):
        ...
        try:
            ...
            app = create_ui_app(path, ROLES_PATH)
            client = TestClient(app)
            resp = client.post("/api/messages", json={"content": "hello ui"})
            assert resp.status_code == 200
            data = resp.json()
            assert data["content"] == "hello ui"
            assert data["sender"] == "user"

            msgs = client.get("/api/messages").json()
            assert len(msgs) == 1
            assert msgs[0]["content"] == "hello ui"
        finally:
            os.unlink(path)
    ...
    def test_get_messages_since(self):
        ...
        try:
            ...
            app = create_ui_app(path, ROLES_PATH)
            client = TestClient(app)
            r1 = client.post("/api/messages", json={"content": "one"}).json()
            client.post("/api/messages", json={"content": "two"})
            client.post("/api/messages", json={"content": "three"})
            since = client.get(f"/api/messages/since/{r1['id']}").json()
            assert len(since) == 2
            assert since[0]["content"] == "two"
        finally:
            os.unlink(path)

    def test_channel_isolation_via_api(self):
        from fastapi.testclient import TestClient
        import tempfile, os
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
            path = f.name
        try:
            ...
            client.post("/api/messages", json={"content": "in design", "channel": "design"})
            general = client.get("/api/messages?channel=general").json()
            design = client.get("/api/messages?channel=design").json()
            assert len(general) == 1
            assert general[0]["content"] == "in general"
            assert len(design) == 1
            assert design[0]["content"] == "in design"
        finally:
            os.unlink(path)
